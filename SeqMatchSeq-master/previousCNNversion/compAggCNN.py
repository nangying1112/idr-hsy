...
class compAggWikiqa(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.rl_soft_module = TempNet(2*mem_dim)
        ...
        self.aa_conv_module = self.new_conv_module()
        self.aa_soft_module = TempNet(mem_dim)
    ...
    def comp_agg(self,data_q,data_as):
    ...
    def aa_comp_agg(self,data_q,data_as):
        data_as_len = torch.cuda.IntTensor(len(data_as))
        for k in range(len(data_as)):
            data_as_len[k] = data_as[k].size()[0]
            if data_as_len[k] < self.window_large:
        ...
        sim_output = self.sim_sg_module.forward(projs_a_emb, att_output)
        # print('sim_output',sim_output.size())
        conv_output = self.aa_conv_module.forward(sim_output, data_as_len)
        # print('conv_output',conv_output.size())
        soft_output = self.aa_soft_module.forward(conv_output)
        return conv_output, soft_output


    def rl_state(self,data_q,data_as,q_a_state, q_a_score):
        ...
        ref_answer = data_q
        current_answer = [data_as[0]]
        ref_info, _ = self.comp_agg(ref_answer, current_answer)
        ...
        for k in range(2, len(data_as) + 1):
            ...
            if pos_a_index == k - 1:
                cur_ref_info,_ = self.aa_comp_agg(data_q, current_answer)
                qa_info = torch.cat([qa_info, cur_qa_info], 0)
                ref_info = torch.cat([ref_info, cur_ref_info], 0)
            else:
                ref_answer = data_as[pos_a_index]
                current_answer = [data_as[k - 1]]
                cur_ref_info, _ = self.aa_comp_agg(ref_answer, current_answer)
                cur_ref_info = cur_ref_info.view(1, q_a_state[0].size(0))
                qa_info = torch.cat([qa_info, cur_qa_info], 0)
                ref_info = torch.cat([ref_info, cur_ref_info], 0)
    ...
    def forward(self, data_q, data_as):
        ...
        conv_output,soft_output = self.comp_agg(data_q,data_as)
        # get rl state
        rl_state = self.rl_state(data_q,data_as,conv_output,soft_output)
