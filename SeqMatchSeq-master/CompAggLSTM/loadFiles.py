        ...
        for folder, filename in filenames.items():
            ...
            for line in open(filename):
                ...
                if curr_q_words == question:  # the candidates this line is same with previous
                    ...
                    if not all(labels):
                        pass
                    if sum(labels) > 0:  # if True, put previous instance into data; you need at least one positive label
                        question_ = torch.LongTensor(question)
                        # pad the answer ,the seq length is the length of the longest candidate answer.
                        answers_length = [len(answer) for answer in candidates]
                        max_length = max(answers_length)
                        for i in range(len(candidates)):
                            candidates[i] = candidates[i] + [0 for j in range(max_length - len(candidates[i]))]
                        # print(max_length)
                        candidates_np = np.array(candidates)
                        candidates_ = torch.LongTensor(candidates_np)
                        # print(candidates_.size())
                        # candidates_ = [torch.LongTensor(words) for words in candidates]
                        labels_ = torch.FloatTensor(labels)
                        labels_ = labels_.type(torch.FloatTensor)
                        data.append((question_, candidates_, answers_length, labels_))
