...
import numpy as np
from agu2_pdtb_data import pddata
...
flags.DEFINE_integer('rnn_size', 200, 'hidden_units_size of lstm')
...
class idr_base_model():
    ...
    def __init__(self, batch_size, learning_rate, vocabulary_size, embedding_size, rnn_size, gcn_size, clip_value, epoch,
        ...
        # GCN
        self.A_matrix = self.get_A_matrix(arg1_outputs, arg2_outputs)
        self.X_matrix = arg2_outputs
        W1 = tf.Variable(tf.truncated_normal([rnn_size*2, gcn_size], stddev=0.1))
        ...
        for i in range(self.input_size):
            ...
            # convolve
            # print(self.A_matrix[i].shape,pre_sup.shape)
            output = tf.matmul(self.A_matrix[i], pre_sup)
        ...
        for epoch in range(1, epoch):
            ...
            for iteration in range(iterations):
                ...
                if step % 10 == 0:
                    ...
                    # pre_pos_num = len([_ for pre_class in predict if pre_class==1])
                    # pre_neg_num = len([_ for pre_class in predict if pre_class == 0])
                    # print(pre_pos_num, pre_neg_num)
    ...
    def get_A_matrix(self, agu1, agu2):
        ...
        for i in range(self.input_size):
            ...
            score_ij = tf.nn.softmax(
                tf.matmul(agu1_fw_bw, agu2_fw_bw, transpose_a=False, transpose_b=True))  # score_ij :[seq_length,seq_length]
